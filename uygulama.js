...
// Token'lı dosya indirme (rapor uçları girisGerekli ister)
function raporIndir(yol) {
  window.open(API + yol + (yol.includes('?') ? '&' : '?') + 'token=' + durum.token, '_blank');
}

async function raporlarSayfa(hedef) {
  hedef.innerHTML = `
    <div class="sayfa-baslik">
      <div><h1>Raporlar</h1><p>Evrak durumu raporlarını indirin veya yazdırın</p></div>
    </div>
    <div class="rapor-izgara">
      <div class="rapor-kart">
        <div class="bik">📊</div>
        ...
        <div class="indir">
          <button class="btn btn-mini" data-html="/api/rapor/uygunluk.html">🖨️ Yazdır / PDF</button>
          <button class="btn btn-acik btn-mini" data-csv="/api/rapor/uygunluk.csv">⬇️ Excel (CSV)</button>
        </div>
      </div>
      <div class="rapor-kart">
        <div class="bik">⚠️</div>
        <h3>Eksik Belgeler</h3>
        <p>Hangi hak sahibinin hangi belgesi eksik veya süresi dolmuş — satır satır liste.</p>
        <div class="indir">
          <button class="btn btn-acik btn-mini" data-csv="/api/rapor/eksik-belgeler.csv">⬇️ Excel (CSV)</button>
        </div>
      </div>
    </div>
    <div class="mesaj-bilgi">💡 "Yazdır / PDF" seçeneği yeni sekmede açılır; oradan tarayıcınızın yazdır penceresiyle PDF olarak kaydedebilirsiniz. CSV dosyaları Excel'de açılır.</div>`;

  hedef.querySelectorAll('[data-html]').forEach(b => b.addEventListener('click', () => raporIndir(b.dataset.html)));
  hedef.querySelectorAll('[data-csv]').forEach(b => b.addEventListener('click', () => raporIndir(b.dataset.csv)));
}

async function mesajlarSayfa(hedef) {
  try {
    const veri = await istek('/mesaj/kayitlar');
    hedef.innerHTML = `
      <div class="sayfa-baslik">
        <div><h1>Toplu Mesaj</h1><p>Hak sahiplerine SMS veya e-posta gönderin</p></div>
        <button class="btn" id="yeni-mesaj">+ Mesaj gönder</button>
      </div>
      ${veri.simulasyon ? '<div class="mesaj-bilgi">⚙️ <b>Simülasyon modu:</b> Mesajlar kaydedilir ama gerçekten gönderilmez. Gerçek gönderim için bir SMS/e-posta sağlayıcısı (Netgsm, SMTP vb.) bağlanmalıdır.</div>' : ''}
      <div class="panel">
        <div class="panel-baslik"><h2>Gönderim geçmişi</h2></div>
        ${veri.kayitlar.length === 0
          ...
          : veri.kayitlar.map(m => `
            <div class="mesaj-kayit">
              <div class="bilgi">
                <b>${m.kanal === 'sms' ? '📱 SMS' : '✉️ E-posta'} → ${escapeHtml(m.alici_ad || m.alici)}</b>
                <p>${escapeHtml(m.icerik)}</p>
              </div>
              <span class="mesaj-durum ${m.durum}">${m.durum === 'gonderildi' ? 'Gönderildi' : m.durum === 'basarisiz' ? 'Başarısız' : 'Kuyrukta'}</span>
            </div>`).join('')}
...
function mesajFormu() {
  modal('Toplu mesaj gönder', `
    <div class="alan">
      ...
      <div class="mesaj-kanal-sec">
        <label><input type="radio" name="kanal" value="sms" checked /> <span class="bik">📱</span> SMS</label>
        <label><input type="radio" name="kanal" value="eposta" /> <span class="bik">✉️</span> E-posta</label>
      </div>
    </div>
    <div class="alan">
      <label>Kimlere?</label>
    ...
    <div class="alan eposta-konu gizli"><label>Konu (e-posta)</label><input name="konu" placeholder="Mesaj konusu" /></div>
    <div class="alan"><label>Mesaj *</label><textarea name="icerik" rows="4" required placeholder="ör. Sayın hak sahibi, eksik belgelerinizi en kısa sürede yüklemenizi rica ederiz."></textarea></div>
  `, async (form) => {
    const govde = formVeri(form);
    const sonuc = await istek('/mesaj/toplu', { method: 'POST', body: JSON.stringify(govde) });
    const ek = sonuc.simulasyon ? ' (simülasyon)' : '';
    toast(`${sonuc.gonderildi} mesaj gönderildi${sonuc.basarisiz ? ', ' + sonuc.basarisiz + ' başarısız' : ''}${ek}.`);
    sayfaCiz();
  });

  // E-posta seçilince konu alanını göster
  const form = document.getElementById('modal-form');
  const konuAlan = form.querySelector('.eposta-konu');
  form.querySelectorAll('[name="kanal"]').forEach(r => r.addEventListener('change', () => {
    konuAlan.classList.toggle('gizli', form.querySelector('[name="kanal"]:checked').value !== 'eposta');
  }));
}
